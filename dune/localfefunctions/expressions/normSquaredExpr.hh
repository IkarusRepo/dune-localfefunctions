    ...
    auto evaluateDerivativeOfExpression(const LFArgs &lfArgs) const {
      ...
      if constexpr (DerivativeOrder == 1)  // d(squaredNorm(u))/dx = 2 * u_x * u
      {
        ...
        if constexpr (LFArgs::hasOneSpatialSingle) {
          ...
          return Dune::eval(2 * (u_xyzAlongu + leftMultiplyTranspose(u_xy, u_z) + leftMultiplyTranspose(u_xz, u_y) + u_yzAlongux));
          ...
          for (int i = 0; i < gridDim; ++i)
            res += (leftMultiplyTranspose(u_c1, gradu_c0[i]) + leftMultiplyTranspose(u_c0, gradu_c1[i])) * coeff(alongMatrix,0, i);
