...
namespace Dune {
  ...
  Dune::BlockVector<Type>& addInEmbedding(Dune::BlockVector<Type>& a, const Eigen::MatrixBase<Derived>& b) requires(
      Dune::Concepts::AddAssignAble<Type, decltype(b.template segment<Type::valueSize>(0))>and requires() {
        Type::valueSize;
      }) {
  ...
  auto transpose(const Eigen::EigenBase<Derived>& A) {
    ...
    else if constexpr (Dune::Std::IsSpecializationTypeAndNonTypes<Eigen::DiagonalMatrix, Derived>::value
                       or Std::isSpecialization<Eigen::DiagonalWrapper, Derived>::value)
      return A.derived();
    else
      static_assert((requires { A.derived().transpose(); })
                    or Dune::Std::IsSpecializationTypeAndNonTypes<Eigen::DiagonalMatrix, Derived>::value
                    or Std::isSpecialization<Eigen::DiagonalWrapper, Derived>::value);
  }
