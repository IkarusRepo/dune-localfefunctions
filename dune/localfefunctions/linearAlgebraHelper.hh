...
namespace Dune {
   ...
   * */
  template <typename field_type, int rows, int cols>
  auto leftMultiplyTranspose(const Dune::FieldVector<field_type, rows>& x, const Dune::FieldMatrix<field_type, rows,cols>& A) {
  ...
  auto leftMultiplyTranspose(const Dune::FieldVector<field_type, size>& x, const Dune::ScaledIdentityMatrix<field_type, size>& A) {
  ...
  template <typename field_type, int size>
  auto leftMultiplyTranspose(const Dune::ScaledIdentityMatrix<field_type, size>& B, const Dune::ScaledIdentityMatrix<field_type, size>& A) {
    auto y = B;
    y.scalar()*=A.scalar();
  ...
  template <typename field_type, int size>
  auto leftMultiplyTranspose(const Dune::FieldVector<field_type, size>& x, const Dune::DiagonalMatrix<field_type, size>& A) {
  ...
  template <typename field_type, int size>
  auto leftMultiplyTranspose(const Dune::FieldVector<field_type, size>& x, const Dune::FieldVector<field_type, size>& a) {
  ...
  template <typename field_type, int rows, int cols>
  auto leftMultiplyTranspose(const Dune::FieldMatrix<field_type, rows, cols>& A, const Dune::FieldVector<field_type, rows>& x) {
    Dune::FieldMatrix<field_type, cols,1> y;
    A.mtv(x,y);  // y=A x
    return y;
  }
  ...
  auto leftMultiplyTranspose(const Dune::FieldMatrix<field_type, rows,cols1>& B, const Dune::FieldMatrix<field_type, rows,cols2>& A) {
  ...
  auto leftMultiplyTranspose(const Dune::FieldMatrix<field_type, rows,cols1>& B, const Dune::DiagonalMatrix<field_type, rows>& A) {
    Dune::FieldMatrix<field_type, cols1,rows> BT;
    Dune::MatrixVector::transpose(B,BT);
  ...
  template <typename field_type, int rows>
  auto operator+(const Dune::FieldMatrix<field_type, rows,rows>& B, const Dune::ScaledIdentityMatrix<field_type, rows>& A) {
    auto y=B;
  ...
  template <typename field_type, int rows>
  auto operator+(const Dune::ScaledIdentityMatrix<field_type, rows>& B, const Dune::ScaledIdentityMatrix<field_type, rows>& A) {
    auto y=B;
    y.scalar()+=A.scalar();
  ...
  template <typename field_type, int rows>
  Dune::DiagonalMatrix<field_type, rows> leftMultiplyTranspose(const Dune::DiagonalMatrix<field_type, rows>& B, const Dune::DiagonalMatrix<field_type, rows>& A) {
  ...
  /** \brief Return a static sized block of a EigenMatrix  (lower1...lower1+size1-1,lower2...lower2+size2-1) as FieldMatrix */
  ...
  template <typename field_type, int rows, int cols>
  auto leftMultiplyTranspose(const Dune::FieldMatrix<field_type, rows,cols>& , const Dune::DerivativeDirections::ZeroMatrix& ) {

    return Dune::DerivativeDirections::ZeroMatrix();
  }

  template <typename field_type, int rows, int cols>
  auto leftMultiplyTranspose(const Dune::DerivativeDirections::ZeroMatrix& ,const Dune::FieldMatrix<field_type, rows,cols>& ) {

    return Dune::DerivativeDirections::ZeroMatrix();
  }

  template <typename field_type, int rows>
  auto leftMultiplyTranspose(const Dune::DerivativeDirections::ZeroMatrix& ,const Dune::DiagonalMatrix<field_type, rows>& ) {

    return Dune::DerivativeDirections::ZeroMatrix();
  }

  template <typename field_type, int rows>
  auto leftMultiplyTranspose(const Dune::DerivativeDirections::ZeroMatrix& ,const Dune::ScaledIdentityMatrix<field_type, rows>& ) {

    return Dune::DerivativeDirections::ZeroMatrix();
  }

  template <typename field_type, int rows>
  auto leftMultiplyTranspose(const Dune::DiagonalMatrix<field_type, rows>& ,const Dune::DerivativeDirections::ZeroMatrix& A) {

    return Dune::DerivativeDirections::ZeroMatrix();
  }

  template <typename field_type, int rows>
  auto leftMultiplyTranspose(const Dune::ScaledIdentityMatrix<field_type, rows>& ,const Dune::DerivativeDirections::ZeroMatrix& A) {
  ...
  /** \brief Returns a reference to a row. This is easily done since Dune  simply returns a reference to the stacked FieldVector
   * But again to stay compatible with a possible Eigen replacement we can not use operator[] for both cases*/
  template <typename field_type, int rows, int cols>
   ...
   * Dune FieldMatrix provides [i][j] for element access but this is not provided by Eigen::Matrix. Thus we fallback to have this implemented by an indirection
  ...
  auto viewAsEigenMatrixFixedDyn(Dune::BlockVector<Dune::FieldVector<ScalarType,1>>& blockedVector) {
    Eigen::Map<Eigen::Matrix<typename Dune::FieldVector<ScalarType,1>::field_type, Dune::FieldVector<ScalarType,1>::valueSize, Eigen::Dynamic>> vec(
        (&blockedVector.begin()->begin().operator*())[0], blockedVector[0].size(), blockedVector.size());
  ...
  auto viewAsEigenMatrixFixedDyn(const Dune::BlockVector<Dune::FieldVector<ScalarType,1>>& blockedVector) {
    Eigen::Map<const Eigen::Matrix<typename Dune::FieldVector<ScalarType,1>::field_type, Dune::FieldVector<ScalarType,1>::valueSize, Eigen::Dynamic>> vec(
        (&blockedVector.begin()->begin().operator*())[0], blockedVector[0].size(), blockedVector.size());
  ...
  Dune::BlockVector<Type>& operator+=(Dune::BlockVector<Type>& a, const Eigen::MatrixBase<Derived>& b)
    requires(Dune::Concepts::AddAssignAble<Type, decltype(segmentToDune<Type::correctionSize>(b,0))>
             and requires() { Type::correctionSize; })
  {
  ...
  template <typename Type, typename Derived>
  Dune::BlockVector<Type>& operator-=(Dune::BlockVector<Type>& a, const Eigen::MatrixBase<Derived>& b)
    requires(Dune::Concepts::AddAssignAble<Type, decltype(b.template segment<Type::correctionSize>(0))>
             and requires() { Type::correctionSize; })
  {
    return a += (-b);
  }
  ...
  template <typename Scalar, int size>
  Dune::FieldMatrix<Scalar, size>& operator-=(Dune::FieldMatrix<Scalar, size>& a, const Dune::ScaledIdentityMatrix<Scalar, size>& b) {
    for (int i = 0; i < size; ++i)
      a[i][i]-=b.scalar();
  ...
  template <typename Scalar, int size>
  Eigen::DiagonalMatrix<Scalar, size> operator-(const Eigen::DiagonalMatrix<Scalar, size>& a, Dune::DerivativeDirections::ZeroMatrix) {
    return a.derived();
  }

  template <typename Scalar, int size>
  Dune::DiagonalMatrix<Scalar, size> operator-(const Dune::DiagonalMatrix<Scalar, size>& a, Dune::DerivativeDirections::ZeroMatrix) {
    return a;
  }
  ...
  template <typename Scalar, int size>
  Eigen::DiagonalMatrix<Scalar, size> operator-(Dune::DerivativeDirections::ZeroMatrix, const Eigen::DiagonalMatrix<Scalar, size>& a) {
    return -a.derived();
  }

  template <typename Scalar, int size>
  Dune::DiagonalMatrix<Scalar, size> operator-(Dune::DerivativeDirections::ZeroMatrix, const Dune::DiagonalMatrix<Scalar, size>& a) {
    return -a;
  }
