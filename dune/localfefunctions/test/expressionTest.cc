...
//        static_assert(spatialSingleDerivative.rows == localFunctionValueSize);
      ...
      auto spatialDerivAll = [&](auto &gpOffset_) {
        return toEigen(lf.evaluateDerivative(toFieldVector(gpOffset_), Dune::wrt(spatialAll),
      ...
      for (int i = 0; i < gridDim; ++i) {
        ...
        auto derivDerivSingleI = [&](auto gpOffset_) {
          ...
          return toEigen(lf.evaluateDerivative(toFieldVector(offSetSingle), Dune::wrt(spatial(i)),
                                                     Dune::on(DerivativeDirections::referenceElement)));
        };
        ...
        auto funcSingle = [&](const auto &gpOffset_) {
          ...
          offSetSingle[i] += gpOffset_[0];
          return toEigen(
              lf.evaluate(toFieldVector(offSetSingle), Dune::on(DerivativeDirections::referenceElement)));
        };
        ...
        try {
          ...
          std::cout
              << "Single SpatialDerivative not tested, since it is not implemented by the local function "  + ("(" +localFunctionName+ ")")
              << std::endl;
        }
      }
    }
    ...
    const Dune::FieldVector<double, localFunctionValueSize> alongVec
        = localFunctionValueSize == 1 ? createOnesVector<double, localFunctionValueSize>()
                                      : createRandomVector<Dune::FieldVector<double, localFunctionValueSize>>();
    const Dune::FieldMatrix<double, localFunctionValueSize, gridDim> alongMat
        = localFunctionValueSize == 1
              ? createOnesMatrix<double, localFunctionValueSize, gridDim>()
              : createRandomVector<Dune::FieldMatrix<double, localFunctionValueSize, gridDim>>();
    ...
    try {
      ...
      std::cout << "Single SpatialDerivative not tested, since it is not implemented by the local function "  + ("(" +localFunctionName+ ")")
                << std::endl;
    ...
    for (size_t i = 0; i < coeffSize; ++i) {
      ...
      static_assert(jacobianWRTCoeffslf.rows == localFunctionValueSize);
      static_assert(jacobianWRTCoeffslf.cols == coeffCorrectionSize);
      const auto jacobianWRTCoeffs = transposeEvaluated(leftMultiplyTranspose(alongVec, jacobianWRTCoeffslf));
      static_assert(jacobianWRTCoeffs.rows == coeffCorrectionSize);  // note the  transpose above
      static_assert(jacobianWRTCoeffs.cols == 1);
      auto jacobianWRTCoeffsAD = transposeEvaluated(BLAi) * segmentToDune<coeffValueSize>(gradienWRTCoeffs, i * coeffValueSize);
      t.require(
          isApproxSame(jacobianWRTCoeffs,
                       jacobianWRTCoeffsAD, tol),
          "Test first derivative wrt coeff");

      if (spatialSingleImplemented)
        for (int d = 0; d < gridDim; ++d) {
          const auto jacoWrtCoeffAndSpatiallf = lf.evaluateDerivative(ipIndex, Dune::wrt(coeff(i), spatial(d)),
                                                                      Dune::on(DerivativeDirections::referenceElement));
          static_assert(jacoWrtCoeffAndSpatiallf.cols == coeffCorrectionSize);
          static_assert(jacoWrtCoeffAndSpatiallf.rows == localFunctionValueSize);
          ...
          t.check(
              isApproxSame(jacoWrtCoeffAndSpatial,
                           transposeEvaluated(BLAi)
                               * segmentToDune<coeffValueSize>(gradientWRTCoeffsTwoTimesSingleSpatial[d], i * coeffValueSize),
                           tol),
      ...
      if (spatialAllImplemented and spatialSingleImplemented) {
        ...
        static_assert(jacoWrtSpatialAllAndCoeff[0].cols == coeffCorrectionSize);
        static_assert(jacoWrtSpatialAllAndCoeff[0].rows == localFunctionValueSize);

        Dune::FieldMatrix<double, 1, coeffCorrectionSize> jacoWrtSpatialAllAndCoeffProd;
        jacoWrtSpatialAllAndCoeffProd = 0;
      ...
      for (size_t j = 0; j < coeffSize; ++j) {
        ...
        const auto jacobianWRTCoeffsTwoTimesExpected = Dune::eval(
            transposeEvaluated(BLAi)
                * blockToDune<coeffValueSize, coeffValueSize>(hessianWRTCoeffs, i * coeffValueSize, j * coeffValueSize) * BLAj
        ...
        t.check(passed,
                "Test second derivatives wrt coeffs" +std::string((i == j) ? " for i==j " : " for i!=j"));
        if(not passed) {
          std::cout << "Actual: \n"
                    << jacobianWRTCoeffsTwoTimes << "\n Expected: \n"
                    << jacobianWRTCoeffsTwoTimesExpected << "\n Diff: \n"
                    << jacobianWRTCoeffsTwoTimes - jacobianWRTCoeffsTwoTimesExpected << "\n Weingarten part: \n"
                    << (i == j) * coeffs[i].weingartenMap(segmentToDune<coeffValueSize>(gradienWRTCoeffs, i * coeffValueSize))<<std::endl;
        }
        ...
        if (spatialAllImplemented) {
          ...
          const auto jacobianWRTCoeffsTwoTimesSpatialAllExpected = Dune::eval(
              transposeEvaluated(BLAi)
                  * blockToDune<coeffValueSize, coeffValueSize>(hessianWRTCoeffsSpatialAll, i * coeffValueSize,
                                                          j * coeffValueSize)
                  * BLAj
              + (i == j)
                    * coeffs[j].weingartenMap(segmentToDune<coeffValueSize>(gradienWRTCoeffsSpatialAll, i * coeffValueSize)));
          ...
          if constexpr (lf.order() < quadratic) {
            ...
            const bool passed = isApproxSame(jacobianWRTCoeffsTwoTimesSpatialAll, jacobianWRTCoeffsTwoTimesSpatialAllExpected, tol);
            t.check(passed,
                    "Test third derivatives wrt coeffs, coeffs and spatialall");
            if(not passed)
              std::cout<<"Actual: \n"<<jacobianWRTCoeffsTwoTimesSpatialAll<<"\n Expected: \n"<<jacobianWRTCoeffsTwoTimesSpatialAllExpected<<"\n Diff: \n"<<jacobianWRTCoeffsTwoTimesSpatialAll-jacobianWRTCoeffsTwoTimesSpatialAllExpected<<std::endl;
          }
        }
        if (spatialSingleImplemented)
          for (int d = 0; d < gridDim; ++d) {
            ...
            const auto jacobianWRTCoeffsTwoTimesSingleSpatialExpected
                = Dune::eval(transposeEvaluated(BLAi)
                                 * blockToDune<coeffValueSize, coeffValueSize>(hessianWRTCoeffsTwoTimesSingleSpatial[d],
            ...
            const bool passed = isApproxSame(jacobianWRTCoeffsTwoTimesSingleSpatial, jacobianWRTCoeffsTwoTimesSingleSpatialExpected,
                                             tol);
            t.check(passed,
                    "Test third derivatives wrt coeffs, coeffs and spatial single");
            if(not passed)
              std::cout<<"Actual: \n"<<jacobianWRTCoeffsTwoTimesSingleSpatial<<"\n Expected: \n"<<jacobianWRTCoeffsTwoTimesSingleSpatialExpected<<"\n Diff: \n"<<jacobianWRTCoeffsTwoTimesSingleSpatial-jacobianWRTCoeffsTwoTimesSingleSpatialExpected<<std::endl;
          }
      }
    }
  }
...
auto localFunctionTestConstructor(const Dune::GeometryType &geometryType, size_t nNodalTestPointsI = 1) {
  ...
  using VectorType   = Dune::FieldVector<double, worldDim>;
  using MatrixType   = Dune::FieldMatrix<double, worldDim, worldDim>;
  ...
  for (int gpIndex = 0; [[maybe_unused]] auto &gp : rule) {
    ...
    for (size_t iC = 0; iC < fe.size(); ++iC) {
      const VectorType dfdi = g2.evaluate(gpIndex) * N[iC];
