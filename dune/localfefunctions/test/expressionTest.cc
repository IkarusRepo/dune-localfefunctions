...
//#include <Dune/linearAlgebra/nonLinearOperator.hh>
...
//#include <dune/localfefunctions/functionSanityChecks.hh>
...
void testLocalFunction(const LF& lf) {
  ...
  std::cout<<"Testing: " + std::string(isCopy ? "Copy " : "") + Dune::localFEFunctionName(lf));
  ...
  for (const auto& [ipIndex, ip] : lf.viewOverIntegrationPoints()) {
    ...
    {
      const auto spatialAllDerivative = lf.evaluateDerivative(ipIndex, Dune::wrt(spatialAll));
      static_assert(spatialAllDerivative.cols() == gridDim);
      static_assert(spatialAllDerivative.rows() == localFunctionValueSize);
      const auto spatialSingleDerivative = lf.evaluateDerivative(ipIndex, Dune::wrt(spatial(0)));
...
int main(int argc, char** argv) try {
 Dune::MPIHelper::instance(argc, argv);
  ...
  using Manifold      = Dune::RealTuple<double, sizeD>;
  using Manifold2     = Dune::UnitVector<double, sizeD>;
  ...
  for (auto& ele : elements(gridView)) {
    ...
    auto localBasis  = Dune::LocalBasis(localView.tree().finiteElement().localBasis());
    ...
    auto f = Dune::StandardLocalFEFunction(localBasis, vBlockedLocal);
    {
      auto localBasisNotBound = Dune::LocalBasis(localView.tree().finiteElement().localBasis());
      auto fNotBound          = Dune::StandardLocalFEFunction(localBasisNotBound, vBlockedLocal);
      auto h                  = f + fNotBound;
      ...
      auto localBasisBoundButToDifferentRule = Dune::LocalBasis(localView.tree().finiteElement().localBasis());
      localBasisBoundButToDifferentRule.bind(ruleHigher, bindDerivatives(0, 1));
      auto fBoundButHigher = Dune::StandardLocalFEFunction(localBasisBoundButToDifferentRule, vBlockedLocal);
      auto h2              = f + fBoundButHigher;
      EXPECT_DEBUG_DEATH(h.viewOverIntegrationPoints(), "The basis of the leaf nodes are not in the same state.");
    }
    static_assert(f.order() == linear);
    auto g = Dune::StandardLocalFEFunction(localBasis, vBlockedLocal);
    ...
    //                                                              Dune::Arithmetic, Dune::index_constant<0>>>);
    ...
    auto f2 = Dune::StandardLocalFEFunction(localBasis, vBlockedLocal, _0);
    auto g2 = Dune::StandardLocalFEFunction(localBasis, vBlockedLocal2, _1);
