  ...
  for (const auto &[ipIndex, ip] : lf.viewOverIntegrationPoints()) {
    ...
    /// Check spatial derivatives return sizes
    {
      ...
      auto spatialDerivAll
          = [&](auto &gpOffset_) { return lf.evaluateDerivative(toFieldVector(gpOffset_), Dune::wrt(spatialAll)); };

      auto derivDerivSingle = [&](auto gpOffset_, int spatialIndex) {
        return lf.evaluateDerivative(toFieldVector(gpOffset_), Dune::wrt(spatial(spatialIndex)));
      };
...
//            nonLinOpSpatialAll, {.draw = false, .writeSlopeStatementIfFailed = true, .tolerance = 1e-2})));
      ...
      for (int i = 0; i < gridDim; ++i) {
        ...
        auto derivDerivSingleI = [&](auto gpOffset_) {
          ...
          return lf.evaluateDerivative(toFieldVector(offSetSingle), Dune::wrt(spatial(i)));
        };
...
//              nonLinOpSpatialSingle, {.draw = false, .writeSlopeStatementIfFailed = true, .tolerance = 1e-2})));
    ...
    for (size_t i = 0; i < coeffSize; ++i) {
      ...
      const auto jacobianWRTCoeffslf = lf.evaluateDerivative(ipIndex, Dune::wrt(coeff(i)));
      ...
      testSuite.require(isApproxSame(jacobianWRTCoeffs,BLAi.transpose() * gradienWRTCoeffs.template segment<coeffValueSize>(i * coeffValueSize), tol));
      ...
      if (spatialSingleImplemented)
        for (int d = 0; d < gridDim; ++d) {
          ...
          const auto jacoWrtSpatialAndCoeff
              = ((alongVec.transpose() * lf.evaluateDerivative(ipIndex, Dune::wrt(spatial(d), coeff(i)))).transpose())
                    .eval();
          ...
          testSuite.require(isApproxSame(jacoWrtCoeffAndSpatial,jacoWrtSpatialAndCoeff, tol));
          testSuite.require(isApproxSame(jacoWrtCoeffAndSpatial,BLAi.transpose()
                                                                     * gradientWRTCoeffsTwoTimesSingleSpatial[d].template segment<coeffValueSize>(
                                                                         i * coeffValueSize), tol));
        }
      ...
      if (spatialAllImplemented and spatialSingleImplemented) {
        ...
        testSuite.require(isApproxSame(jacoWrtSpatialAllAndCoeffProd,(BLAi.transpose()
* gradienWRTCoeffsSpatialAll.template segment<coeffValueSize>(i * coeffValueSize))
   .transpose(), tol));
        ...
        for (int j = 0; j < gridDim; ++j)
          testSuite.require(isApproxSame(Warray[j],Warray2[j], tol));
        ...
        for (int j = 0; j < gridDim; ++j)
          testSuite.require(isApproxSame(Warray[j],WarraySingle[j], tol));

      }
      for (size_t j = 0; j < coeffSize; ++j) {
        const auto BLAj = coeffs[j].orthonormalFrame();
        const auto jacobianWRTCoeffsTwoTimes
            = lf.evaluateDerivative(ipIndex, Dune::wrt(coeff(i, j)), Dune::along(alongVec));
        ...
        const auto jacobianWRTCoeffsTwoTimesExpected
            = (BLAi.transpose()
               ...
               + (i == j)
                  ...
                  .eval();
        testSuite.require(isApproxSame(jacobianWRTCoeffsTwoTimes,jacobianWRTCoeffsTwoTimesExpected, tol));
        ...
        if (spatialAllImplemented) {
          ...
          if constexpr (LF::order() < quadratic) {
            ...
            testSuite.require(isApproxSame(jacobianWRTCoeffsTwoTimesSpatialAll,jacobianWRTCoeffsTwoTimesSpatialAllExpected, tol));
          }
        }
        if (spatialSingleImplemented)
          for (int d = 0; d < gridDim; ++d) {
            ...
            const auto jacobianWRTCoeffsTwoTimesSingleSpatialExpected
                = (BLAi.transpose()
                       ...
                       * BLAj
                   + (i == j) //Correction due to curvature of the manifold
                         * coeffs[j].weingartenMap(
                             gradientWRTCoeffsTwoTimesSingleSpatial[d].template segment<coeffValueSize>(
                                 i * coeffValueSize)))
                      .eval();
            testSuite.require(isApproxSame(jacobianWRTCoeffsTwoTimesSingleSpatial,jacobianWRTCoeffsTwoTimesSingleSpatialExpected, tol));
          }
      }
    }
  }
  ...
  if (not isCopy) {  //  test the cloned local function
    ...
    for (size_t i = 0; i < coeffSize; ++i)
      testSuite.require(coeffCopy[i] == coeffs[i]);
...
void localFunctionTestConstructor(const Dune::GeometryType &geometryType, size_t nNodalTestPointsI = 1) {
  ...
  for (size_t k = 0; k < fe.size(); ++k) {
    testSuite.require(h.coefficientsRef(_0)[k] == vBlockedLocal[k]);
    testSuite.require(h.coefficientsRef(_1)[k] == vBlockedLocal[k]);
  }
  ...
  for (int gpIndex = 0; auto &gp : rule) {
    ...
    testSuite.require(Dune::FloatCmp::eq((f2.evaluateFunction(gpIndex) + g2.evaluateFunction(gpIndex)).dot(g2.evaluateFunction(gpIndex)),(k2.evaluateFunction(gpIndex)[0])));
    auto resSingleSpatial
        = ((f2.evaluateDerivative(gpIndex, wrt(spatial(0))) + g2.evaluateDerivative(gpIndex, wrt(spatial(0))))
           ...
           + (f2.evaluateFunction(gpIndex) + g2.evaluateFunction(gpIndex)).transpose()
              ...
              .eval();
    testSuite.require(isApproxSame(resSingleSpatial,k2.evaluateDerivative(gpIndex, wrt(spatial(0))), tol));
    ...
    testSuite.require(isApproxSame(resSpatialAll,k2.evaluateDerivative(gpIndex, wrt(spatialAll)), tol));
    ...
    for (size_t iC = 0; iC < fe.size(); ++iC) {
      ...
      testSuite.require(isApproxSame(dfdi,dkdi, tol));
      ...
      for (size_t jC = 0; jC < fe.size(); ++jC) {
        ...
        testSuite.require(isApproxSame(dkdijExpected,dkdij, tol));
        ...
        testSuite.require(isApproxSame(dkdijExpected2,dkdij2, tol));
        ...
        testSuite.require(isApproxSame(dkdijExpected3,dkdij3, tol));
        ...
        testSuite.require(isApproxSame(dkdSijR,dkdSij, tol));

        testSuite.require(isApproxSame(dkdSijExpected,dkdSij, tol));
      }
    }
    ++gpIndex;
  }
...
void testOnTriangle() {
  ...
  localFunctionTestConstructor<2, 2, 1>(triangle);
...
void testOnQuadrilateral() {
  ...
  localFunctionTestConstructor<2, 2, 1>(quadrilateral);
